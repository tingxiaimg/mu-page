((typeof self !== 'undefined' ? self : this)["webpackJsonpmuPage"] = (typeof self !== 'undefined' ? self : this)["webpackJsonpmuPage"] || []).push([[0,1],{

/***/ "034f":
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
/* harmony import */ var _node_modules_vue_style_loader_index_js_ref_6_oneOf_1_0_node_modules_css_loader_index_js_ref_6_oneOf_1_1_node_modules_vue_loader_lib_loaders_stylePostLoader_js_node_modules_postcss_loader_src_index_js_ref_6_oneOf_1_2_node_modules_postcss_loader_src_index_js_ref_6_oneOf_1_3_node_modules_cache_loader_dist_cjs_js_ref_0_0_node_modules_vue_loader_lib_index_js_vue_loader_options_App_vue_vue_type_style_index_0_lang_css___WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__("e2a1");
/* harmony import */ var _node_modules_vue_style_loader_index_js_ref_6_oneOf_1_0_node_modules_css_loader_index_js_ref_6_oneOf_1_1_node_modules_vue_loader_lib_loaders_stylePostLoader_js_node_modules_postcss_loader_src_index_js_ref_6_oneOf_1_2_node_modules_postcss_loader_src_index_js_ref_6_oneOf_1_3_node_modules_cache_loader_dist_cjs_js_ref_0_0_node_modules_vue_loader_lib_index_js_vue_loader_options_App_vue_vue_type_style_index_0_lang_css___WEBPACK_IMPORTED_MODULE_0___default = /*#__PURE__*/__webpack_require__.n(_node_modules_vue_style_loader_index_js_ref_6_oneOf_1_0_node_modules_css_loader_index_js_ref_6_oneOf_1_1_node_modules_vue_loader_lib_loaders_stylePostLoader_js_node_modules_postcss_loader_src_index_js_ref_6_oneOf_1_2_node_modules_postcss_loader_src_index_js_ref_6_oneOf_1_3_node_modules_cache_loader_dist_cjs_js_ref_0_0_node_modules_vue_loader_lib_index_js_vue_loader_options_App_vue_vue_type_style_index_0_lang_css___WEBPACK_IMPORTED_MODULE_0__);
/* unused harmony reexport * */
 /* unused harmony default export */ var _unused_webpack_default_export = (_node_modules_vue_style_loader_index_js_ref_6_oneOf_1_0_node_modules_css_loader_index_js_ref_6_oneOf_1_1_node_modules_vue_loader_lib_loaders_stylePostLoader_js_node_modules_postcss_loader_src_index_js_ref_6_oneOf_1_2_node_modules_postcss_loader_src_index_js_ref_6_oneOf_1_3_node_modules_cache_loader_dist_cjs_js_ref_0_0_node_modules_vue_loader_lib_index_js_vue_loader_options_App_vue_vue_type_style_index_0_lang_css___WEBPACK_IMPORTED_MODULE_0___default.a); 

/***/ }),

/***/ "3dfd":
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);

// CONCATENATED MODULE: ./node_modules/cache-loader/dist/cjs.js?{"cacheDirectory":"node_modules//.cache//vue-loader","cacheIdentifier":"1ed55e01-vue-loader-template"}!./node_modules/vue-loader/lib/loaders/templateLoader.js??vue-loader-options!./node_modules/cache-loader/dist/cjs.js??ref--0-0!./node_modules/vue-loader/lib??vue-loader-options!./src/App.vue?vue&type=template&id=2ba6b92a&
var render = function () {var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;return _c('div',{attrs:{"id":"app"}},[_c('div',[_c('mu-page-link',{attrs:{"me":"open","to":"test1"}},[_vm._v("test1")]),_c('mu-page-link',{attrs:{"me":"open","to":"test2"}},[_vm._v("test2")]),_c('mu-page-link',{attrs:{"me":"open","to":"test3"}},[_vm._v("test3")]),_c('mu-page-link',{attrs:{"me":"open","to":"test4"}},[_vm._v("test4")]),_c('mu-page-link',{attrs:{"me":"open","to":"test5"}},[_vm._v("test5")])],1),_c('div',[_c('mu-page-link',{attrs:{"to":"test1","data":'test1当前时间是' + Date.now()}},[_vm._v("test1")]),_c('mu-page-link',{attrs:{"to":"test2","data":'test2当前时间是' + Date.now()}},[_vm._v("test2")]),_c('mu-page-link',{attrs:{"to":"test3","data":'test3当前时间是' + Date.now()}},[_vm._v("test3")]),_c('mu-page-link',{attrs:{"to":"test4","data":'test4当前时间是' + Date.now()}},[_vm._v("test4")]),_c('mu-page-link',{attrs:{"to":"test5","data":'test5当前时间是' + Date.now()}},[_vm._v("test5")])],1),_c('div',[_c('span',{on:{"click":function($event){_vm.close('test1')}}},[_vm._v("关闭test1(主页无法关闭)")]),_c('span',{on:{"click":function($event){_vm.close('test2')}}},[_vm._v("关闭test2")]),_c('span',{on:{"click":function($event){_vm.close('test3')}}},[_vm._v("关闭test3")]),_c('span',{on:{"click":function($event){_vm.close('test4')}}},[_vm._v("关闭test4")]),_c('span',{on:{"click":function($event){_vm.close('test5')}}},[_vm._v("关闭test5")]),_c('span',{on:{"click":_vm.closeOther}},[_vm._v("关闭其它")]),_c('span',{on:{"click":function($event){_vm.close('*')}}},[_vm._v("关闭所有")])]),_c('div',[_c('span',[_vm._v("打开页面：")]),_vm._l((_vm.visitedViews),function(item){return _c('span',[_vm._v(_vm._s(item.name))])})],2),_c('div',{staticStyle:{"background-color":"#cccccc","border":"1px solid #4cce72"}},[_c('mu-page')],1)])}
var staticRenderFns = []


// CONCATENATED MODULE: ./src/App.vue?vue&type=template&id=2ba6b92a&

// CONCATENATED MODULE: ./node_modules/cache-loader/dist/cjs.js??ref--12-0!./node_modules/thread-loader/dist/cjs.js!./node_modules/babel-loader/lib!./node_modules/cache-loader/dist/cjs.js??ref--0-0!./node_modules/vue-loader/lib??vue-loader-options!./src/App.vue?vue&type=script&lang=js&
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
/* harmony default export */ var Appvue_type_script_lang_js_ = ({
  name: 'app',
  computed: {
    visitedViews: function visitedViews() {
      return this.$muPage.visitedViews;
    }
  },
  methods: {
    close: function close(name) {
      this.$muPage.close(name);
    },
    closeOther: function closeOther() {
      this.$muPage.closeOther();
    }
  }
});
// CONCATENATED MODULE: ./src/App.vue?vue&type=script&lang=js&
 /* harmony default export */ var src_Appvue_type_script_lang_js_ = (Appvue_type_script_lang_js_); 
// EXTERNAL MODULE: ./src/App.vue?vue&type=style&index=0&lang=css&
var Appvue_type_style_index_0_lang_css_ = __webpack_require__("034f");

// EXTERNAL MODULE: ./node_modules/vue-loader/lib/runtime/componentNormalizer.js
var componentNormalizer = __webpack_require__("2877");

// CONCATENATED MODULE: ./src/App.vue






/* normalize component */

var component = Object(componentNormalizer["a" /* default */])(
  src_Appvue_type_script_lang_js_,
  render,
  staticRenderFns,
  false,
  null,
  null,
  null
  
)

component.options.__file = "App.vue"
/* harmony default export */ var App = __webpack_exports__["default"] = (component.exports);

/***/ }),

/***/ "3f8c":
/***/ (function(module, exports, __webpack_require__) {

exports = module.exports = __webpack_require__("2350")(false);
// imports


// module
exports.push([module.i, "#app{font-family:Avenir,Helvetica,Arial,sans-serif;-webkit-font-smoothing:antialiased;-moz-osx-font-smoothing:grayscale;text-align:center;color:#2c3e50;margin-top:60px}span{margin:0 5px;cursor:pointer}", ""]);

// exports


/***/ }),

/***/ "56d7":
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var vue__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__("8bbf");
/* harmony import */ var vue__WEBPACK_IMPORTED_MODULE_0___default = /*#__PURE__*/__webpack_require__.n(vue__WEBPACK_IMPORTED_MODULE_0__);
/* harmony import */ var _App_vue__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__("3dfd");
/* harmony import */ var _index_js__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__("b635");
/* harmony import */ var _components_error_vue__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__("c359");




vue__WEBPACK_IMPORTED_MODULE_0___default.a.config.productionTip = false;
vue__WEBPACK_IMPORTED_MODULE_0___default.a.use(_index_js__WEBPACK_IMPORTED_MODULE_2__["default"]);
var muPage = new _index_js__WEBPACK_IMPORTED_MODULE_2__["default"]({
  components: {
    test1: 'testViews/test1.vue',
    test2: 'testViews/test2.vue',
    test3: 'testViews/test3.vue',
    test4: 'testViews/test4.vue',
    test5: 'testViews/test5.vue'
  },
  homePage: 'test1',
  errorPage: _components_error_vue__WEBPACK_IMPORTED_MODULE_3__["default"]
});
new vue__WEBPACK_IMPORTED_MODULE_0___default.a({
  muPage: muPage,
  render: function render(h) {
    return h(_App_vue__WEBPACK_IMPORTED_MODULE_1__["default"]);
  }
}).$mount('#app');

/***/ }),

/***/ "e2a1":
/***/ (function(module, exports, __webpack_require__) {

// style-loader: Adds some css to the DOM by adding a <style> tag

// load the styles
var content = __webpack_require__("3f8c");
if(typeof content === 'string') content = [[module.i, content, '']];
if(content.locals) module.exports = content.locals;
// add the styles to the DOM
var add = __webpack_require__("499e").default
var update = add("1f50beb8", content, true, {"sourceMap":false,"shadowMode":false});

/***/ })

}]);
//# sourceMappingURL=muPage.common.0.js.map